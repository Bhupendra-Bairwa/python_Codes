...
def add_data():
    ...
    my_name = t1.get("1.0", END).strip()  # read name
    my_class = options.get().strip()  # read class
    my_mark = t3.get("1.0", END).strip()  # read mark
    my_gender = radio_v.get().strip()  # read gender
    ...
    if flag_validation:
        ...
        try:
            query = "INSERT INTO  `Student_detail` (`name` ,`class` ,`mark` ,`gender`) VALUES(%s,%s,%s,%s)"
            ...
            l5.grid()
            l5.config(fg="green", bg="white")
            my_str.set("ID:" + str(last_id))
